...
import { Section } from 'ephox/snooker/api/Structs';
...
UnitTest.asynctest('Redraw Section Order Test', (success, failure) => {
  ...
  const changeTableSections = (table: Element<any>, rowIndex: number, newSection: Section) => {
  ...
  Pipeline.async({}, [
    Logger.t('Render table default options', Step.sync(() => {
      ...
      Assertions.assertStructure('Thead should have become part of tbody again', ApproxStructure.build((s, str, _arr) => s.element('table', {
        children: [
          s.element('tbody', {
            children: [
              basicRowStructure(s, str, 'two'),
